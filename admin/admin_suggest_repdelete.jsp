...
<%
	...
	List<sug_AnswerDto> list = (List<sug_AnswerDto>) request.getAttribute("list");
...
<title>(관리자용)건의사항 게시판 댓글 삭제</title>
   ...
   <section class="bbs_section layout_padding">
      <div class="container_padding" align="center">
			...
			<div class="deltbcontainer">
				<table id="tblist">
					<col width="100"><col width="120"><col width="500"><col width="150">
						<tr class="thtitle">
						...
						<tr class='listtr'><td colspan="3">작성된 글이 없습니다</td></tr>
							<%
								...
								sug_AnswerDto dto = list.get(i); %>
						<tr class='listtr'>
							<td class="chkinput"><input type="checkbox" name="delchk"value='<%=dto.getSuggest_Seq() %>' ></td>
							<td><%=dto.getAnswer_Id() %></td>
...
$("#schbtn").click(function() {
	...
	location.href="./admincont?work=sugrep_qsearch&need="+need+"&fneed="+fneed+"&pageNumber=0&how="+how;
...
function goPage(data) {
	...
	if('<%=need%>'==""){
		location.href="./admincont?work=sugrep_moveQnadel&pageNumber="+data+"&how="+how;
	}else{
		location.href="./admincont?work=sugrep_qsearch&need=<%=need%>&fneed=<%=fneed%>&pageNumber="+data+"&how="+how;
	}
	
}
function fPage(){
	location.href="./admincont?work=sugrep_moveQnadel&pageNumber="+0+"&how="+how;
}
function lpage(){
	//console.log(<%=tpage%>);
	location.href="./admincont?work=sugrep_moveQnadel&pageNumber=<%=tpage-1%>&how="+how;
}
function prevpage(data) {
	location.href="./admincont?work=sugrep_moveQnadel&pageNumber="+((data*5)-5)+"&how="+how;
}
...
function delBtn() {
	...
	//alert(a);
	 $.ajax({
			...
			type:"get", 
			datatype:"text",
			data:"work=sugrep_delchklist&chklist="+a, 
