   ...
   <section class="bbs_section layout_padding">
      <div class="container_padding" align="center">
			...
			<div class="deltbcontainer">
				<table id="tblist">
				<col width="100px"> <col width="500px"><col width="250px">
					<tr class="thtitle">
					...
					<tr class='listtr'>
						...
						<td><input type="text" value="<%=dto.getTitle() %>" style="border:none;" onclick="showCont(<%=dto.getSeq()%>)"></td>
						<td><%=dto.getWdate() %></td>
					</tr>
					<tr class='listtr' style='display:none;' id=<%=dto.getSeq() %>>
						<td><input type="text" style="border:none;" value="▶ 내용 : " size="5"></td>
						<td colspan="2"><input type="text" style="border:none; width:500px; font-size:9pt; word-break:break-all;" value="<%=dto.getContent() %>"> </td>
						<%--<td><textarea style="width:100%; height:100%;" class="itext" ><%=dto.getContent() %></textarea> </td> --%>
					</tr>
						<% } } %>
				</table>
			</div>
...
$("#schbtn").click(function() {
	...
	location.href="./admincont?work=info_qsearch&need="+need+"&fneed="+fneed+"&pageNumber=0&how="+how;
...
function goPage(data) {
	//console.log(data);
	if('<%=need%>'==""){
		location.href="./admincont?work=info_moveQnadel&pageNumber="+data+"&how="+how;
	}else{
		location.href="./admincont?work=info_qsearch&need=<%=need%>&fneed=<%=fneed%>&pageNumber="+data+"&how="+how;
	}
	
}
function fPage(){
	location.href="./admincont?work=info_moveQnadel&pageNumber="+0+"&how="+how;
}
function lpage(){
	//console.log(<%=tpage%>);
	location.href="./admincont?work=info_moveQnadel&pageNumber=<%=tpage-1%>&how="+how;
}
function prevpage(data) {
	location.href="./admincont?work=info_moveQnadel&pageNumber="+((data*5)-5)+"&how="+how;
}
...
function delBtn() {
	...
	if(a==""){
	 ...
	 $.ajax({
			...
			type:"get", 
			datatype:"text",
			data:"work=info_delchklist&chklist="+a, 
	...
	//console.log(a);
	//location.href="./admincont?work=delchklist&chklist="+a;
}
function backBtn() {
