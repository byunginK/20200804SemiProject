...
<%
	...
	List<blistDto> list = (List<blistDto>) request.getAttribute("list");
...
<!-- 게시판 시작 -->
   ...
   <section class="bbs_section layout_padding">
      <div class="container_padding" align="center">
			...
			<th class="table-header">마지막수정일</th>
			</tr>
			...
			<% }
			   if(endBlock > tpage){
				...
				if(pageNumber == i){
			%>
				<li class="page_li"><span style="font-weight:bold; color:#9d6b53 font-size:18pt;"><%=i+1 %>&nbsp;&nbsp;</span></li>
				<%}else{ %>
...
$("#schbtn").click(function() {
	...
	location.href="./admincont?work=qsearch&need="+need+"&fneed="+fneed+"&pageNumber=0&how="+how;
...
function goPage(data) {
	//console.log(data);
	if('<%=need%>'==""){
		location.href="./admincont?work=moveQnadel&pageNumber="+data+"&how="+how;
	}else{
		location.href="./admincont?work=qsearch&need=<%=need%>&fneed=<%=fneed%>&pageNumber="+data+"&how="+how;
	}
	
}
function fPage(){
	location.href="./admincont?work=moveQnadel&pageNumber="+0+"&how="+how;
}
function lpage(){
	//console.log(<%=tpage%>);
	location.href="./admincont?work=moveQnadel&pageNumber=<%=tpage-1%>&how="+how;
}
function prevpage(data) {
	location.href="./admincont?work=moveQnadel&pageNumber="+((data*5)-5)+"&how="+how;
}
...
function delBtn() {
	...
	if(a==""){
	 ...
	 $.ajax({
			...
			type:"get", 
			datatype:"text",
			data:"work=delchklist&chklist="+a, 
